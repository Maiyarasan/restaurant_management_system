...
const SideBar = () => {
    ...
    return (
        <>
            <button className="absolute fixed z-50 p-5 top-4 left-4" onClick={handleNav}>
                {sideBar ? <IoClose size={30} className="text-blue-500" /> : <FaBars size={30} className="text-blue-500" />}
            </button>
            <div ref={sideBarRef} className={`fixed left-0 top-0 w-[75%] sm:w-[60%] md:w-[45%] lg:w-[30%] h-full bg-blue-300 p-5 pt-24 transform ${sideBar ? "translate-x-0" : "-translate-x-full"} transition-transform duration-300 ease-in-out`}>
